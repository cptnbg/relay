...
(`plugins/relay/skills/relay/SKILL.md:278-317`), which first resolves the
...
run is already active, or consent was never recorded (`SKILL.md:280-283`),
...
(`SKILL.md:291-292`). Because a detached launch that dies at preflight is
...
(`SKILL.md:296-309`). From this point the interactive session is gone; the
...
3. Refuse if the plan file named by `plan_path` does not exist — a session
   ...
   and the handoff (`:246-261`) — and refuse a `model_tier` outside
...
5. Acquire the single-instance-per-project lock
   (`relay-supervisor.sh:340-345`, via `relay_lock` in
   `lib/relay-lib.sh:212-251`) and install signal traps (`:346`).
6. Run `relay-doctor.sh` as a preflight gate; on failure, exit 78 without
   starting anything (`:351-355`). Doctor itself never mutates anything
...
7. If a previous run already measured this project's context baseline,
   ...
   soft threshold (`relay-supervisor.sh:357-377`) — see §6.
8. Prune old session logs with `relay_prune_sessions` (`:382`,
...
9b. Validate `sandbox_mode` as exactly `enforced` or `disabled` and journal it
   (`:405-427`). Anything else refuses at preflight, like an invalid
...
10. Build the `--settings` JSON payload (`:437-438`, see §7) and **prove** the
   ...
   proved it (`:440-500`). Because the mode changes the payload, it also changes
   ...
   skipped the proof (`:445-458`). `relay_settings_probe()`
   (`relay-settings.sh:494-594`) appends a relay-owned
   ...
   (`relay-settings.sh:619-738`) runs instead: with no sandbox, an unreadable
   ...
   *accepted* — by supplying the environment relay's own inline hook needs
   ...
   (`relay-settings.sh:424-454`) parses the `code=`/`rc=` markers — never
...
11. Self-test the injection and guardrail-drift filters against the live
   ...
   the highest-priority injection halt into a silent no-op (`:628-668`).
12. Hash the plan file and record `status: running` through `state_set`
    (`relay-supervisor.sh:502-503`).

Only then, after the `supervisor.start` journal line, does the `while :; do`
loop start (`:949-951`). On its first pass the
pre-spawn gates (`:952-976`, see §3) all pass trivially — `COMPLETE.md`,
`BLOCKED.md`, and `STOP` do not exist yet. The loop consumes any queued
`INBOX.md` note (`:981-989`), increments the session counter to 1, and — with
`review_every` at its default of 5 — the review-cadence check at `:996-1005`
does not fire on session 1, so `MODE` stays `normal`. `build_prompt` is
called (`:1019`, defined `:721-806`); since `$STATE/work/continue.json` does
not exist yet, `handoff_valid` returns false (`:549-561`) and the prompt's
untrusted-handoff block reads literally `(no valid handoff; this is the first
session or the last one failed)` (`:795`). The supervisor then asserts its
own argv never contains a forbidden flag and always contains the two required
ones (`:1023-1026`, `relay-settings.sh:355-378`) before exec'ing the first
`claude -p` session under `--permission-mode dontAsk`
(`relay-supervisor.sh:1034-1051`).

### 1b. From one session ending to the next starting

When `claude` exits, the supervisor captures its exit code and wall-clock
duration into `RC` and `DUR` (`:1052-1054`), adds the envelope's `total_cost_usd` to the running
total (`:1084-1086`), extracts a human-readable reason
from the session's own JSON envelope for the journal (`:1056-1064`), and
records this session's context-at-rest baseline if one was observed
(`:1066-1082`, see §6). It then runs the full post-exit predicate chain
described in §3. If nothing in that chain halts the run, the loop falls
through to persisting its counters with `state_set` (`:1299-1303`), sleeps
`RELAY_POLL_INTERVAL` seconds (default 5, `:1305`), and loops back to
`while :; do` (`:951`) — this time
...
(`:1056-1060`). That is why the supervisor extracts `session.reason` from the
...
- **Sealed sentinel files.** `sealed()` requires both that the file exists
  ...
  (`relay-supervisor.sh:809`). An unsealed file is a half-written file, and
  acting on one races a session that may still be writing it (`:806-808`).
...
- **Commit counts.** `verify_complete()` compares the repository's current
  ...
  of the run (`:824-827`). The count is a *proxy* for "did anything happen",
  ...
  commits is rejected (`:918-922`); with one configured, the acceptance
  ...
  than silently waved through (`:910-916`). Both numbers are normalised
...
- **Working-tree cleanliness.** `verify_complete()` also requires
  `git status --porcelain` to be empty (`:813-815`) — a claimed completion
  with uncommitted changes lying around is rejected.
- **The handoff hash.** `PREV_HANDOFF`/`NEW_HANDOFF` are content hashes of
  `continue.json` taken before and after the session (`:979,1168`), used both
  to decide whether to archive a new handoff (`:1191-1196`) and, together
  with the commit-count comparison, to decide whether the session was
  productive at all (`:1219-1222`).
...
nobody approved (`:841-916`).

## 3. The post-exit predicate order

The block the supervisor labels `# ---- post-exit predicates, in order ----`
begins at `relay-supervisor.sh:1088`. In exact source order:

1. **`work/COMPLETE.md` sealed** (`:1090`). If sealed, `verify_complete()` is
   called (`:811-924`, see §2). If it passes: journal, notify, set
   `status: complete`, exit `EX_OK` (0) (`:1091-1095`). If it fails
   verification, `COMPLETE.md` is deleted so the next session must earn it
   again (`:1097-1099`), a `complete_rejections` counter is incremented
   (`:1101-1102`), and at 3 rejections the run exits `EX_REJECTED` (27)
   (`:1103-1107`); otherwise `NEXT_TIER` is set to `fable` — "it thinks it is
   done and it is not: escalate judgment" (`:1108`).
2. **`work/BLOCKED.md` sealed** (`:1111-1116`). Journal, notify, exit
   `EX_BLOCKED` (20).
3. **`STOP` file present** (`:1118-1121`). Exit `EX_STOPPED` (24).
4. **Usage-limit detection** (`:1123-1161`). `usage_limited()` reads only the
   ...
   cannot be mistaken for one that hit one (`:671-715`) — and the envelope is
   captured to a variable before matching, never piped through
   `jq | head | grep -q`, which under `pipefail` could miss a genuine 429
   (`:699-710`). On a genuine limit, the supervisor backs off and retries up
   to `max_usage_retries` (default 20) without consuming the session counter
   or the stall/fastfail streaks (`:1126-1160`), puts a consumed-but-unread
   operator note back into `INBOX.md` so the retry does not destroy it
   (`inbox.preserved-on-retry`, `:1136-1146`), and polls `STOP` during an
   adaptively-stepped backoff sleep (`:1147-1157`) — or exits `EX_BUDGET` (29)
   if `on_limit` is not `wait` or retries are exhausted (`:1127-1131`).

Only after these four checks does the supervisor compute the new commit head
and normalize/hash the handoff (`:1164-1168`). Then:

5. **Guardrail drift in the handoff** (`:1170-1185`). The comment at
   `:1170-1171` states the reasoning directly: "Guardrail drift beats
   ...
   (`:591-626`) — the supervisor
   writes a fresh `work/BLOCKED.md` explaining what triggered it and exits
   `EX_BLOCKED` (`:1184`) — *before* the handoff is archived and *before*
   ...
   any lasting effect.
6. Lower-confidence injection matches are logged, not halted (`:1187-1189`).
7. If the handoff changed and is structurally valid, archive a copy under
   `$STATE/handoffs/` — never overwriting a prior one (`:1191-1196`).
8. **Commit the session's work** via `relay_git_commit()`
   (`relay-supervisor.sh:1207`, `relay-git.sh:241-341`). If a high-confidence
   ...
   (`relay-supervisor.sh:1209-1213`, `relay-git.sh:311-332`). A commit that
   ...
   (`relay-supervisor.sh:1198-1216`).
9. Compute `PRODUCTIVE` from whether `HEAD` moved or the handoff changed
   validly (`relay-supervisor.sh:1219-1222`).
10. Reset `NEXT_MODE` to `normal`, then force `recovery` mode if a
    `PreCompact` event fired this session (`:1224-1229`) or the handoff is
    invalid (`:1230-1233`). Both compaction markers are **deleted** once
    consumed (`:1227`) — the events file is append-only from the hook's side,
...
11. On a session timeout (`RC` 124 or 137), force recovery mode, count
    ...
    `max_timeouts` (default 2) (`:1234-1246`).
12. Update the stall and fastfail streaks. A productive session clears both,
    ...
    `min_session_secs` (`:1251-1266`). Auto-escalate to `fable` one session
    *before* either breaker would trip — "escalate before giving up: a
    smarter model is strictly better than a halt" (`:1267-1274`).
13. Trip the circuit breakers: `stall_limit` sessions with no progress exits
    `EX_STALLED` (21) (`:1277-1281`); `fastfail_limit` consecutive
...
14. **De-escalate**: if the tier is `fable` and this session was productive,
    return to the default tier in one shot (`:1289`) — the `De-escalate`
...
15. Detect and journal a mid-run plan change by comparing plan file hashes
    (`:1291-1297`).
16. Persist all counters with `state_set` and loop (`:1299-1305`).
...
(`relay-supervisor.sh:46-75`, `relay-settings.sh:179-196`). **Under
...
- **`$STATE/work/` — session-writable.** The sandbox's `filesystem` block
  ...
  (`relay-settings.sh:326-328`). Everything a session or the context
...
`sandbox: {"enabled": false}` and nothing else (`relay-settings.sh:310-312`).
The supervisor-only files are named in `permissions.deny` instead
(`relay_settings_deny_for_mode`, `relay-settings.sh:197-235`) — and a
...
session existed (`relay-supervisor.sh:859-863`). Rewriting the command and its
...
|---|---|---|
| `work/RUN.md` | Mission, acceptance criteria, guardrails, decisions already made; written once at `/relay-init`, read by every session (`SKILL.md:164-179`, `relay-supervisor.sh:729`) | Yes — the primary document a human reviews before and during a run |
| `work/continue.json` | The live handoff; see §5 (`relay-supervisor.sh:509`) | Not normally — schema-validated machine state (`SKILL.md:479-480`) |
| `work/HUMAN-TASKS.md` | Non-blocking items a session appended instead of stopping (`relay-supervisor.sh:746-747`); counted at `:1299` | Yes — the intended place a human catches up |
| `work/BLOCKED.md` | Sealed sentinel for a genuine blocker, written by a session (`:748-750`) or by relay itself on guardrail drift (`:1174-1181`), a detected secret (`relay-git.sh:315-330`), or an acceptance-command approval-hash mismatch (`:865-873`) | Yes — read and triaged on `/relay-resume` (`SKILL.md:388-400`) |
| `work/COMPLETE.md` | Sealed sentinel claiming the plan is done, with cited proof (`relay-supervisor.sh:751-753`) | Yes, once verified |
...
| `work/probe/` | The sandbox probe's scratch: `probe-canary.txt`, `probe-read.out` (the canary-leak proof file), `probe-read.json`/`.err`, `probe-egress.txt` (the `code=`/`rc=` egress evidence) (`relay-settings.sh:510-513,563`) | Only when the probe fails |
...
- `ctx.log` — one line per context-guard call that samples the transcript
  ...
  (`relay-supervisor.sh:1073-1082`).
- `hook-<session-id>.state` — the context guard's own throttle state:
  `LAST_EPOCH|LAST_PCT|LAST_LEVEL|CALLS` (`relay-ctx.sh:90-91,138-147,158-167`).
...
| Path | What it is | Human-readable? |
|---|---|---|
| `config.json` | Settings only, never commands (`SKILL.md:197-201`); read via `cfg()` (`relay-supervisor.sh:86-89`); also records `consent` (`SKILL.md:255-271`) | Occasionally, to check caps |
| `exec.json` | The approved `acceptance_cmd` argv array plus its approval hash `exec_hash` (`SKILL.md:422-450`), validated at `relay-supervisor.sh:222-245` and re-verified immediately before the command runs (`:841-877`) | Occasionally, to check what runs |
| `state.json` | Machine state: `run_id`, `status`, `reason`, `session_count`, `stall_count`, `fastfail_streak`, `fable_used`, `next_mode`, `next_tier`, `cost_total`, `ctx_baseline`, `last_review_n`, `complete_rejections`, `commits_at_start`, `plan_hash`, `human_tasks`, `last_session_rc`, `timeouts` (written throughout via `state_set`, e.g. `:112,503,875,1080,1094,1300-1303`) | Yes, via `/relay-status` (`SKILL.md:321-368`) |
| `journal.log` | Tab-separated `epoch\tevent\tdetail` audit trail (`relay_journal`, `lib/relay-lib.sh:43-50`), set as `RELAY_JOURNAL` (`relay-supervisor.sh:77`) | Yes — `tail -f` is the documented way to watch a run (`SKILL.md:314`) |
| `INBOX.md` | Queued operator notes from `/relay-note`, consumed atomically each loop iteration (`SKILL.md:378-384`, `relay-supervisor.sh:981-989`) and restored if a usage-limit retry would have destroyed one unread (`:1136-1146`) | Write-only for a human; not meant to be read back |
| `STOP` | Empty kill-switch file touched by `/relay-stop` (`SKILL.md:370-376`) | No — a marker, not content |
| `supervisor.out` | Redirected stdout/stderr of the supervisor process itself (`SKILL.md:291-292`) | Only when the supervisor itself misbehaves |
...
| `sessions/` | `<NNN>-<uuid>.log` and `.log.err`, redirected as `$SLOG` and `$SLOG.err`, one pair per session (`relay-supervisor.sh:1017,1050`), pruned to `keep_sessions` (default 5) and `keep_days` (default 7) by `relay_prune_sessions` (`relay-supervisor.sh:382`, `lib/relay-lib.sh:555-584`) because logs "hold whatever the agent read" (`lib/relay-lib.sh:541-542`) | Only when diagnosing |
| `handoffs/` | `<NNN>-<hash>.json`, every accepted handoff, archived and never overwritten in place (`relay-supervisor.sh:1191-1196`) | Only when diagnosing |
| `locks/run.d/owner` | `pid|epoch|host|run_id`, the single-instance lock (`lib/relay-lib.sh:230`, `relay-supervisor.sh:340-345`); also how `/relay-status` and `run` establish liveness (`SKILL.md:296-309,343-355`) | Only when diagnosing |
| `run/probe.ok` | The settings-probe fingerprint cache — payload + CLI version, required to be 40-hex (`relay-supervisor.sh:445-490`) | No |
| `run/inbox-current.md` | This iteration's consumed operator note (`relay-supervisor.sh:981-989`), rendered into the prompt (`:798-802`) | No |
| `run/acceptance.log` | stdout/stderr of the acceptance command (`:906`) | On acceptance failures |
...
`handoff_valid()` (`relay-supervisor.sh:549-561`) requires `next` to be a
non-empty array, `done` to be an array, every string in all four arrays to
be at most 280 characters (`:554-555`), at most 12 entries in `done` and at
most 12 in `next` (`:556` — note it bounds only those two arrays, not
...
`handoff_normalize()` (`:536-542`) before this check runs, not by this check. A handoff that fails this shape check outright — not
JSON, missing `next`, wrong types — is dropped, and the next session runs in
recovery mode (`:1230-1233`).

A handoff with the right shape but over-long entries is **not** treated as a
structural failure. `handoff_normalize()` (`:524-547`) truncates any string
over 280 characters and caps each array at 12 entries, rather than
discarding the whole file. The comment at `:511-523` explains why: an
...
`handoff_render()` (`:573-583`) turns the validated JSON into plain,
labeled text (`DONE:` / `NEXT:` / `FILES TOUCHED:` / `OPEN QUESTIONS:`),
strips any line matching the injection heuristic (`INJECTION_RE`, `:571`,
applied at `:581` — its `^` anchors tolerate the `- ` bullet the renderer
itself adds, without which the anchored patterns filtered nothing while the
journal still reported "filtered N", `:563-570`), and strips
...
(`:790-795`). The nonce is regenerated every run specifically so a handoff
cannot carry a stale fence marker forward into a later run (`:332-336`).
...
the mitigation, not an afterthought" (`:505-507`). There is no free-text
...
(`:573-583`), so nothing injected survives into the prompt from any field.
But `handoff_guardrail_drift()` (`:622-626`) and `handoff_flagged_lines()`
(`:585-588`) both read only `done`, `next` and `open_questions` —
...
`relay-supervisor.sh:285`; `plugins/relay/config/defaults.json:15`), and
validated: anything outside `opus|sonnet|fable` refuses at preflight rather
than reaching `--model` unvetted (`:290-297`). Relay
sets the model for the **top-level session** only, via `--model "$TIER"`
(`relay-supervisor.sh:1043`). Subagents that top-level session spawns through
...
(`relay-supervisor.sh:736-739`) — it is a property of how the session is
...
`window_for_tier()` (`:298-304`: `window_sonnet`, `window_fable`, default
...
(`RELAY_MIN_WINDOW`, `:317`) or the supervisor refuses to start
(`:318-330`): a session already holds tens of thousands of tokens of system
prompt, tool definitions, and `CLAUDE.md` before its first tool call — 48,070
measured on one project (`:306-308`) — and a window at or below that floor makes
the context guard report "critical" on the very first call, every session,
forever (`:308-313`). Once a project has completed one session, relay knows
...
left in front of it (`:357-377`) — this is what caught the case where a
120k window against a 69k measured baseline left 3k of working room and two
sessions in a row committed nothing (`:361-362`).

**Escalation.** `NEXT_TIER` starts as the configured default (`:937`) and
...
2. Reaching `STALL_LIMIT - 1` unproductive sessions or
   ...
   `config/defaults.json:19`) is not already exhausted (`:1267-1274`).
3. If a session is asked to run as `fable` but the cap is already
   ...
   notified as `escalation.exhausted` (`:1007-1012`).

**De-escalation is one-shot.** `[ "$NEXT_TIER" = "fable" ] && [ "$PRODUCTIVE" -eq 1 ] && NEXT_TIER="$TIER_DEFAULT"`
(`:1289`) — the very next productive session after an escalation returns
...
comment above the line says the same thing in one sentence (`:1288`).
...
(`docs/security.md:92-94`). This is a design decision, not an omission. It
...
globally (`docs/security.md:86-90`), which means relay never needs a
...
(`relay-settings.sh:336-345` — the `hooks` object, with its `PostToolUse` and
...
containing spaces is never re-parsed by a shell (`relay-settings.sh:335-338`,
corroborated by `docs/security.md:121-123`).
...
(`relay-supervisor.sh:1036-1038`). It validates
`RELAY_SESSION_ID` is
...
- **`--precompact` mode** emits a fixed compaction warning and unconditionally
  ...
  actually trusts (`relay-ctx.sh:102-117`, `relay-supervisor.sh:1224-1229`) —
...
instruction" (`relay-ctx.sh:314-317`). The hook also always exits 0, on every path,
