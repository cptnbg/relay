...
(`plugins/relay/skills/relay/SKILL.md:338-377`), which first resolves the
...
run is already active, or consent was never recorded (`SKILL.md:340-343`),
...
(`SKILL.md:351-352`). Because a detached launch that dies at preflight is
...
(`SKILL.md:356-369`). From this point the interactive session is gone; the
...
3. Refuse if the plan file named by `plan_path` does not exist — a session
   ...
   and the handoff (`:262-312`) — and refuse a `model_tier` outside
...
5. Acquire the single-instance-per-project lock
   (`relay-supervisor.sh:425-430`, via `relay_lock` in
   `lib/relay-lib.sh:212-251`) and install signal traps (`:431`).
6. Run `relay-doctor.sh` as a preflight gate; on failure, exit 78 without
   starting anything (`:436-440`). Doctor itself never mutates anything
...
7. If a previous run already measured this project's context baseline,
   ...
   soft threshold (`relay-supervisor.sh:442-462`) — see §6.
8. Prune old session logs with `relay_prune_sessions` (`:467`,
...
9b. Validate `sandbox_mode` as exactly `enforced` or `disabled` and journal it
   (`:490-573`). Anything else refuses at preflight, like an invalid
...
10. Build the `--settings` JSON payload (`:583-584`, see §7) and **prove** the
   ...
   proved it (`:586-646`). Because the mode changes the payload, it also changes
   ...
   skipped the proof (`:591-604`). `relay_settings_probe()`
...
11. Self-test the injection and guardrail-drift filters against the live
   ...
   the highest-priority injection halt into a silent no-op (`:870-910`).
12. Hash the plan file and record `status: running` through `state_set`
    (`relay-supervisor.sh:648-649`).

Only then, after the `supervisor.start` journal line, does the `while :; do`
loop start (`:1382-1384`). On its first pass the
pre-spawn gates (`:1385-1427`, see §3) all pass trivially — `COMPLETE.md`,
`BLOCKED.md`, and `STOP` do not exist yet. The loop consumes any queued
`INBOX.md` note (`:1432-1440`), increments the session counter to 1, and — with
`review_every` at its default of 5 — the review-cadence check at `:1458-1470`
does not fire on session 1, so `MODE` stays `normal`. `build_prompt` is
called (`:1484`, defined `:963-1129`); since `$STATE/work/continue.json` does
not exist yet, `handoff_valid` returns false (`:699-715`) and the prompt's
untrusted-handoff block reads literally `(no valid handoff; this is the first
session or the last one failed)` (`:1105`). The supervisor then asserts its
own argv never contains a forbidden flag and always contains the two required
ones (`:1490-1493`, `relay-settings.sh:416-439`) before exec'ing the first
`claude -p` session under `--permission-mode dontAsk`
(`relay-supervisor.sh:1501-1518`).

### 1b. From one session ending to the next starting

When `claude` exits, the supervisor captures its exit code and wall-clock
duration into `RC` and `DUR` (`:1519-1521`), adds the envelope's `total_cost_usd` to the running
total (`:1575-1577`), extracts a human-readable reason
from the session's own JSON envelope for the journal (`:1523-1531`), and
records this session's context-at-rest baseline if one was observed
(`:1557-1573`, see §6). It then runs the full post-exit predicate chain
described in §3. If nothing in that chain halts the run, the loop falls
through to persisting its counters with `state_set` (`:1963-1968`), sleeps
`RELAY_POLL_INTERVAL` seconds (default 5, `:1970`), and loops back to
`while :; do` (`:1384`) — this time
...
(`:1523-1527`). That is why the supervisor extracts `session.reason` from the
...
- **Sealed sentinel files.** `sealed()` requires both that the file exists
  ...
  (`relay-supervisor.sh:1239`). An unsealed file is a half-written file, and
  acting on one races a session that may still be writing it (`:1129-1131`).
...
- **Commit counts.** `verify_complete()` compares the repository's current
  ...
  of the run (`:1254-1257`). The count is a *proxy* for "did anything happen",
  ...
  commits is rejected (`:1346-1350`); with one configured, the acceptance
  ...
  than silently waved through (`:1338-1344`). Both numbers are normalised
...
- **Working-tree cleanliness.** `verify_complete()` also requires
  `git status --porcelain` to be empty (`:1243-1245`) — a claimed completion
  with uncommitted changes lying around is rejected.
- **The handoff hash.** `PREV_HANDOFF`/`NEW_HANDOFF` are content hashes of
  `continue.json` taken before and after the session (`:1430, 1728`), used both
  to decide whether to archive a new handoff (`:1751-1836`) and, together
  with the commit-count comparison, to decide whether the session was
  productive at all (`:1859-1862`).
...
nobody approved (`:1292-1344`).

## 3. The post-exit predicate order

The block the supervisor labels `# ---- post-exit predicates, in order ----`
begins at `relay-supervisor.sh:1615`. In exact source order:

1. **`work/COMPLETE.md` sealed** (`:1645`). If sealed, `verify_complete()` is
   called (`:1241-1352`, see §2). If it passes: journal, notify, set
   `status: complete`, exit `EX_OK` (0) (`:1646-1650`). If it fails
   verification, `COMPLETE.md` is deleted so the next session must earn it
   again (`:1652-1654`), a `complete_rejections` counter is incremented
   (`:1656-1657`), and at 3 rejections the run exits `EX_REJECTED` (27)
   (`:1658-1662`); otherwise `NEXT_TIER` is set to `fable` — "it thinks it is
   done and it is not: escalate judgment" (`:1663`).
2. **`work/BLOCKED.md` sealed** (`:1666-1671`). Journal, notify, exit
   `EX_BLOCKED` (20).
3. **`STOP` file present** (`:1673-1676`). Exit `EX_STOPPED` (24).
4. **Usage-limit detection** (`:1678-1721`). `usage_limited()` reads only the
   ...
   cannot be mistaken for one that hit one (`:913-957`) — and the envelope is
   captured to a variable before matching, never piped through
   `jq | head | grep -q`, which under `pipefail` could miss a genuine 429
   (`:941-952`). On a genuine limit, the supervisor backs off and retries up
   to `max_usage_retries` (default 20) without consuming the session counter
   or the stall/fastfail streaks (`:1681-1720`), puts a consumed-but-unread
   operator note back into `INBOX.md` so the retry does not destroy it
   (`inbox.preserved-on-retry`, `:1691-1701`), and polls `STOP` during an
   adaptively-stepped backoff sleep (`:1702-1717`) — or exits `EX_BUDGET` (29)
   if `on_limit` is not `wait` or retries are exhausted (`:1682-1686`).

Only after these four checks does the supervisor compute the new commit head
and normalize/hash the handoff (`:1724-1728`). Then:

5. **Guardrail drift in the handoff** (`:1730-1745`). The comment at
   `:1730-1731` states the reasoning directly: "Guardrail drift beats
   ...
   (`:823-868`) — the supervisor
   writes a fresh `work/BLOCKED.md` explaining what triggered it and exits
   `EX_BLOCKED` (`:1744`) — *before* the handoff is archived and *before*
   ...
   any lasting effect.
6. Lower-confidence injection matches are logged, not halted (`:1747-1749`).
7. If the handoff changed and is structurally valid, archive a copy under
   `$STATE/handoffs/` — never overwriting a prior one (`:1751-1836`).
8. **Commit the session's work** via `relay_git_commit()`
   (`relay-supervisor.sh:1847`, `relay-git.sh:241-341`). If a high-confidence
   ...
   (`relay-supervisor.sh:1849-1853`, `relay-git.sh:311-332`). A commit that
   ...
   (`relay-supervisor.sh:1838-1856`).
9. Compute `PRODUCTIVE` from whether `HEAD` moved or the handoff changed
   validly (`relay-supervisor.sh:1859-1862`).
10. Reset `NEXT_MODE` to `normal`, then force `recovery` mode if a
    `PreCompact` event fired this session (`:1874-1886`) or the handoff is
    invalid (`:1887-1890`). Both compaction markers are **deleted** once
    consumed (`:1884`) — the events file is append-only from the hook's side,
...
11. On a session timeout (`RC` 124 or 137), force recovery mode, count
    ...
    `max_timeouts` (default 2) (`:1891-1903`).
12. Update the stall and fastfail streaks. A productive session clears both,
    ...
    `min_session_secs` (`:1908-1923`). Auto-escalate to `fable` one session
    *before* either breaker would trip — "escalate before giving up: a
    smarter model is strictly better than a halt" (`:1924-1931`).
13. Trip the circuit breakers: `stall_limit` sessions with no progress exits
    `EX_STALLED` (21) (`:1934-1938`); `fastfail_limit` consecutive
...
14. **De-escalate**: if the tier is `fable` and this session was productive,
    return to the default tier in one shot (`:1946`) — the `De-escalate`
...
15. Detect and journal a mid-run plan change by comparing plan file hashes
    (`:1948-1961`).
16. Persist all counters with `state_set` and loop (`:1963-1970`).
...
session existed (`relay-supervisor.sh:1310-1314`). Rewriting the command and its
...
|---|---|---|
| `work/RUN.md` | Mission, acceptance criteria, guardrails, decisions already made; written once at `/relay-init`, read by every session (`SKILL.md:201-216`, `relay-supervisor.sh:979`) | Yes — the primary document a human reviews before and during a run |
| `work/continue.json` | The live handoff; see §5 (`relay-supervisor.sh:655`) | Not normally — schema-validated machine state (`SKILL.md:603-604`) |
| `work/HUMAN-TASKS.md` | Non-blocking items a session appended instead of stopping (`relay-supervisor.sh:1012-1013`); counted at `:1963` | Yes — the intended place a human catches up |
| `work/BLOCKED.md` | Sealed sentinel for a genuine blocker, written by a session (`:1014-1016`) or by relay itself on guardrail drift (`:1734-1741`), a detected secret (`relay-git.sh:315-330`), or an acceptance-command approval-hash mismatch (`:1316-1324`) | Yes — read and triaged on `/relay-resume` (`SKILL.md:462-487`) |
| `work/COMPLETE.md` | Sealed sentinel claiming the plan is done, with cited proof (`relay-supervisor.sh:1017-1019`) | Yes, once verified |
...
| Path | What it is | Human-readable? |
|---|---|---|
| `config.json` | Settings only, never commands (`SKILL.md:257-261`); read via `cfg()` (`relay-supervisor.sh:86-89`); also records `consent` (`SKILL.md:315-331`) | Occasionally, to check caps |
| `exec.json` | The approved `acceptance_cmd` argv array plus its approval hash `exec_hash` (`SKILL.md:517-545`), validated at `relay-supervisor.sh:238-261` and re-verified immediately before the command runs (`:1292-1328`) | Occasionally, to check what runs |
| `state.json` | Machine state: `run_id`, `status`, `reason`, `session_count`, `stall_count`, `fastfail_streak`, `fable_used`, `next_mode`, `next_tier`, `cost_total`, `ctx_baseline`, `last_review_n`, `complete_rejections`, `commits_at_start`, `plan_hash`, `human_tasks`, `last_session_rc`, `timeouts` (written throughout via `state_set`, e.g. `:112, 649, 1326, 1571, 1649, 1964-1968`) | Yes, via `/relay-status` (`SKILL.md:381-437`) |
...
| `journal.log` | Tab-separated `epoch\tevent\tdetail` audit trail (`relay_journal`, `lib/relay-lib.sh:43-50`), set as `RELAY_JOURNAL` (`relay-supervisor.sh:77`) | Yes — `tail -f` is the documented way to watch a run (`SKILL.md:374`) |
| `INBOX.md` | Queued operator notes from `/relay-note`, consumed atomically each loop iteration (`SKILL.md:452-458`, `relay-supervisor.sh:1432-1440`) and restored if a usage-limit retry would have destroyed one unread (`:1691-1701`) | Write-only for a human; not meant to be read back |
| `STOP` | Empty kill-switch file touched by `/relay-stop` (`SKILL.md:439-450`) | No — a marker, not content |
| `supervisor.out` | Redirected stdout/stderr of the supervisor process itself (`SKILL.md:351-352`) | Only when the supervisor itself misbehaves |
...
| `sessions/` | `<NNN>-<uuid>.log` and `.log.err`, redirected as `$SLOG` and `$SLOG.err`, one pair per session (`relay-supervisor.sh:1482, 1517`), pruned to `keep_sessions` (default 5) and `keep_days` (default 7) by `relay_prune_sessions` (`relay-supervisor.sh:467`, `lib/relay-lib.sh:555-584`) because logs "hold whatever the agent read" (`lib/relay-lib.sh:541-542`) | Only when diagnosing |
| `handoffs/` | `<NNN>-<hash>.json`, every accepted handoff, archived and never overwritten in place (`relay-supervisor.sh:1751-1836`) | Only when diagnosing |
| `locks/run.d/owner` | `pid|epoch|host|run_id`, the single-instance lock (`lib/relay-lib.sh:230`, `relay-supervisor.sh:425-430`); also how `/relay-status` and `run` establish liveness (`SKILL.md:356-369, 403-415`) | Only when diagnosing |
| `run/probe.ok` | The settings-probe fingerprint cache — payload + CLI version, required to be 40-hex (`relay-supervisor.sh:591-636`) | No |
| `run/inbox-current.md` | This iteration's consumed operator note (`relay-supervisor.sh:1432-1440`), rendered into the prompt (`:1121-1125`) | No |
| `run/acceptance.log` | stdout/stderr of the acceptance command (`:1334`) | On acceptance failures |
...
`handoff_valid()` (`relay-supervisor.sh:699-715`) requires `next` to be a
non-empty array, `done` to be an array, every string in all four arrays to
be at most 280 characters (`:707-708`), at most 12 entries in `done` and at
most 12 in `next` (`:709` — note it bounds only those two arrays, not
...
`handoff_normalize()` (`:684-692`) before this check runs, not by this check. A handoff that fails this shape check outright — not
JSON, missing `next`, wrong types — is dropped, and the next session runs in
recovery mode (`:1887-1890`).

A handoff with the right shape but over-long entries is **not** treated as a
structural failure. `handoff_normalize()` (`:670-697`) truncates any string
over 280 characters and caps each array at 12 entries, rather than
discarding the whole file. The comment at `:657-669` explains why: an
...
`handoff_render()` (`:727-739`) turns the validated JSON into plain,
labeled text (`DONE:` / `NEXT:` / `FILES TOUCHED:` / `OPEN QUESTIONS:`),
strips any line matching the injection heuristic (`INJECTION_RE`, `:725`,
applied at `:737` — its `^` anchors tolerate the `- ` bullet the renderer
itself adds, without which the anchored patterns filtered nothing while the
journal still reported "filtered N", `:717-724`), and strips
...
(`:1100-1105`). The nonce is regenerated every run specifically so a handoff
cannot carry a stale fence marker forward into a later run (`:417-421`).
...
the mitigation, not an afterthought" (`:651-653`). There is no free-text
...
(`:727-739`), so nothing injected survives into the prompt from any field.
But `handoff_guardrail_drift()` (`:864-868`) and `handoff_flagged_lines()`
(`:741-820`) both read only `done`, `next` and `open_questions` —
...
`relay-supervisor.sh:370`; `plugins/relay/config/defaults.json:20`), and
validated: anything outside `opus|sonnet|fable` refuses at preflight rather
than reaching `--model` unvetted (`:375-382`). Relay
sets the model for the **top-level session** only, via `--model "$TIER"`
(`relay-supervisor.sh:1510`). Subagents that top-level session spawns through
...
(`relay-supervisor.sh:1002-1005`) — it is a property of how the session is
...
`window_for_tier()` (`:383-389`: `window_sonnet`, `window_fable`, default
`window_opus` — all 200,000 by default, `config/defaults.json:21-23`). Every
configured window is required to be at least 100,000 tokens
(`RELAY_MIN_WINDOW`, `:402`) or the supervisor refuses to start
(`:403-415`): a session already holds tens of thousands of tokens of system
prompt, tool definitions, and `CLAUDE.md` before its first tool call — 48,070
measured on one project (`:391-393`) — and a window at or below that floor makes
the context guard report "critical" on the very first call, every session,
forever (`:393-398`). Once a project has completed one session, relay knows
...
left in front of it (`:442-462`) — this is what caught the case where a
120k window against a 69k measured baseline left 3k of working room and two
sessions in a row committed nothing (`:446-447`).

**Escalation.** `NEXT_TIER` starts as the configured default (`:1370`) and
...
2. Reaching `STALL_LIMIT - 1` unproductive sessions or
   ...
   `config/defaults.json:24`) is not already exhausted (`:1924-1931`).
3. If a session is asked to run as `fable` but the cap is already
   ...
   notified as `escalation.exhausted` (`:1472-1477`).

**De-escalation is one-shot.** `[ "$NEXT_TIER" = "fable" ] && [ "$PRODUCTIVE" -eq 1 ] && NEXT_TIER="$TIER_DEFAULT"`
(`:1946`) — the very next productive session after an escalation returns
...
comment above the line says the same thing in one sentence (`:1945`).
...
(`relay-supervisor.sh:1503-1505`). It validates
`RELAY_SESSION_ID` is
...
hook and stall every tool call in the session (`:186-202`) — and sums the
...
was once measured at 1.36MB (`:208-247`).
...
- **`--precompact` mode** emits a fixed compaction warning and unconditionally
  ...
  actually trusts (`relay-ctx.sh:102-117`, `relay-supervisor.sh:1874-1886`) —
